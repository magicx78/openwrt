...
# Auth-Deskriptor für paramiko-basierte Verbindungen (kein sshpass/subprocess)
...
def _build_base_ssh(ip: str, user: str, password: str, logline):
    """Wählt die beste SSH-Auth-Methode.
    Gibt entweder eine Subprocess-Befehlsliste oder _ParamikoAuth zurück.
    Priorität: sshpass → paramiko → SSH-Key-Auth (subprocess ohne Passwort)."""
    has_sshpass = shutil.which("sshpass") is not None
    if has_sshpass:
...
def _ssh_exec(base_ssh, remote_cmd: str,
    ...
    Akzeptiert eine Subprocess-Befehlsliste oder _ParamikoAuth.
    Ausgaben werden auf _MAX_CMD_OUTPUT Bytes gecappt. Passwort erscheint NICHT im Log."""
    if isinstance(base_ssh, _ParamikoAuth):
...
def _ssh_push_job(job_id: str, ip: str, user: str, password: str, script: str,
    ...
    try:
        ...
        base_ssh = _build_base_ssh(ip, user, password, logline)
...
<table style='width:100%'>
  ...
  <tr><td>🔑 Passwort</td>
      <td><input type='password' id='p-pass' autocomplete='current-password'></td></tr>
</table>
</div>
