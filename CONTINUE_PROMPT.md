# CONTINUE_PROMPT – OpenWrt Provisioning Server v0.5.7

> Stand: 2026-03-02 | Datei: server.py (~6400 Zeilen) | DB: provision.db (SQLite)
...
| SSH-Key Generator & Auto-Installer | OK | NEU v0.5.6: POST /api/ssh/generate-keypair, GET /api/ssh/private-key/download, POST /api/ssh/install-on-device + install-all |
| SSH-Installation parallelisiert | OK | NEU v0.5.7: ThreadPoolExecutor (max 6 Worker), +10x Performance (100s → 15s für 10 Router) |
| Dead Code Cleanup | OK | NEU v0.5.7: 9 doppelte Route-Definitionen entfernt (-1.298 Zeilen, keine funktionalen Änderungen) |
